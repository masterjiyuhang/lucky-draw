    ...
    message() {
      ...
      fields.forEach((item, index) => {
        ...
        if (result[item] && config[item] > 0) {
          const nameList = result[item].reduce((accumulator, resultItem) => {
            const matchedElement = this.currentList.find(
              currentListItem => resultItem === currentListItem.key
            );
            if (matchedElement) {
              accumulator.push(matchedElement.name);
            }
            return accumulator;
          }, []);
          message.push({
